...
(function () {
	...
	function scan_torrent_page(doc, type) {
		...
		torrent_table.find('div.group_info').each(function(i) {
			...
			if (m) {
				...
				if (m) {
					group_id = m[1];
					link = $(this).find('span:first a:first').eq(0);
		...
		function page_handler(response) {
			if (response.status == 200) {
				...
				if (forced_full) {
                    ...
                    if (!lastPage) {
                        $(doc).find('#content .linkbox:last').eq(0).find('a:last').each(function(i) {
                            var pgVal = $(this).attr('href').match(/torrents\.php\?.+&?page=(\d+)/);
                            if (pgVal) {
                                lastPage = pgVal[1];
                            }
                        });
	...
	function mark_snatched_links() {
		...
		if (/\/torrents\.php\?id/.test(document.URL)) {
            ...
            let add_bookmark = $('a.add_bookmark');
            if (!add_bookmark.length) {
                add_bookmark = $('span.add_bookmark');
            }

            let remove_bookmark = $('a.remove_bookmark');
            if (!remove_bookmark.length) {
                remove_bookmark = $('span.remove_bookmark');
            }

			/* show mark/unmark snatched on album page */
			if ((add_bookmark.length || remove_bookmark.length) &&
		...
		if (/\/torrents.php\?id=/.test(document.URL)) {
			...
			if (keys.length) {
				addGroupSnatched(keys[0], snatched_groups[keys[0]], $('#content .header h2')[0]);
			} else {
				var header = $('#content .header h2').text();
				var key = header.replace(releaseTypeRegex,'').replace(/\[\d*\]/, '').trim().toLowerCase();
				if (snatched_groups[key]) {
					addGroupSnatched(key, snatched_groups[key], $('#content .header h2')[0]);
				}
			}
		}
